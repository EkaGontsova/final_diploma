    ...
    * Default `.create()` and `.update()` implementations are provided.
                ...
                if isinstance(
                    ...
                    if constraint.condition is None:
                        ...
                        condition_fields = list(
                            get_referenced_base_fields_from_q(
    ...
    def get_uniqueness_extra_kwargs(
        ...
        for unique_together_list, queryset, condition_fields, condition in self.get_unique_together_constraints(
                model):
            unique_together_list_and_condition_fields = set(
                unique_together_list) | set(condition_fields)
            if model_fields_names.issuperset(
                    unique_together_list_and_condition_fields):
    ...
    def get_unique_together_validators(self):
        ...
        validators = []
        for unique_together, queryset, condition_fields, condition in self.get_unique_together_constraints(
                self.Meta.model):
            # Skip if serializer does not map to all unique together sources
            unique_together_and_condition_fields = set(
                unique_together) | set(condition_fields)
            if not set(source_map).issuperset(
                    unique_together_and_condition_fields):
                continue
            ...
            validator = UniqueTogetherValidator(
                queryset=queryset, fields=field_names, condition_fields=tuple(
                    source_map[f][0] for f in condition_fields), condition=condition, )
